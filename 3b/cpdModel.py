...
class CPDModel():
    def __init__(self, num_agents, num_factions, broadcasts=False, broadcast_percent=None):
        ...
        for i in range(500, 550):
            grp_id = 1
            fac_id = int(i / 25)
            a = AntiAgent(i, grp_id, self)
            self.anti_agents.append(a)
            fac_agent_lists[fac_id].append(a)
            self.agent_list.append(a)
    ...
    def step(self, step_number):
        ...
        if (self.broadcast):
            ...
            if (rand>0.60 and rand<0.61):
                ...
                [broadcasting_agent] = random.sample(possible_broadcasting_agents, 1)
        ...
        if (step_number % logging_granularity == 0):
            ...
            # self.payoff_list[2].append(getAveragePayoff(self.anti_agents))

            self.bias_list[0].append(getAverageBias(self.group_list[0]))
            self.bias_list[1].append(getAverageBias(self.group_list[1]))
            self.bias_list[2].append(getAverageBias(self.anti_agents))
            ...
            # self.facalign_list[2].append(getAverageAlign(self.anti_agents))
