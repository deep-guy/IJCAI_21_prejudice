...
class CPDModel():
    def __init__(self, num_agents, num_factions, broadcasts=False, broadcast_percent=None):
        ...
        # Create agents and divide them in factions
...
print("Running model with 100 precent prejudiced agents")
model_list = []
for r in range(10):
    model = CPDModel(1000, 50, False)
    for i in tqdm(range(1, 100000)):
        model.step(i)
    model_list.append(model)

with open('1a/100.pkl', 'wb') as output:
    pickle.dump(model_list, output, pickle.HIGHEST_PROTOCOL)

print("Model saved as 1a/100.pkl")
print()
